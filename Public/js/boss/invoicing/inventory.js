    ...
    Menu : {
        init : function () {
            ...
            Inventory.productOutside.one('show.bs.modal', function (event) {
                ...
                Inventory.Menu.checkValidate(model, validateParams);
            });
            //单个商品调拨
            ...
            Inventory.productTransferWhere.one('show.bs.modal', function () {
                var models = $(this), validateParams = {};
                    Inventory.Menu.checkValidate(models, validateParams);
        ...
        checkValidate : function (model, validateParams) {
            ...
            $parentBox.validate({
                ...
                submitHandler : function () {
                    if ($parentBox.hasClass('inventory-outside')) {
                        BOSS.Inventory.getAjax(BOSS.api.productOutsides, $parentBox.serialize(), function (d) {
                            if (d.data != null) {
                                BOSS.floatTips.successTips('出库成功！页面将在2秒内刷新');
                                setTimeout(function () {
                        ...
                        BOSS.Inventory.getAjax(BOSS.api.productTransfer, $parentBox.serialize(), function (d) {
                            if (d.data != null) {
                                BOSS.floatTips.successTips('调拨成功！页面将在2秒内刷新');
                                setTimeout(function () {
                                    window.location.reload();
                                }, 2000);
                            } else {
